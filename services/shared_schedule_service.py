...
class SharedScheduleService:
    ...
    def create_schedule(self, data):
        new_schedule = self.repository.create(data)
        # Create alarm entry
        ...
        self.alarm_repository.create(alarm_data)
        return new_schedule
    ...
    def update_schedule(self, id, data):
        ...
        if not schedule:
            return None
        updated_schedule = self.repository.update(schedule, data)
        # Update alarm entry
        alarm = self.alarm_repository.get_by_schedule_id(updated_schedule.id)
        if alarm:
    ...
    def delete_schedule(self, id):
        ...
        alarm = self.alarm_repository.get_by_schedule_id(id)
        if alarm:
